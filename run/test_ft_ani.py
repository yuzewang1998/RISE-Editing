...
def test(model, dataset, visualizer, opt, bg_info, test_steps=0, gen_vid=True, lpips=True,idx = 0):
    print('-----------------------------------Testing-----------------------------------')
    model.eval()
    total_num = dataset.total
    ...
    print("num.{} in {} cases: time used: {} s".format(idx, total_num // opt.test_num_step, time.time() - stime), " at ", visualizer.image_dir)
    visualizer.display_current_results(visuals, idx, opt=opt)
    torch.cuda.empty_cache()
    del visualizer, visuals, gt_image, ray_masks, dataset, model
...
def main():
    ...
    with torch.no_grad():
        ...
        opt.is_train=True
    model = None
    idx = int(opt.resume_iter) - int(opt.start_iter)
    print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    print(idx)
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    ...
    with torch.no_grad():
            ...
            test_opt.split = "test"
            test_opt.name = opt.name + "/test_{}".format(str(int(opt.start_iter)+1))
            test_opt.test_num_step = opt.test_num_step
            test_dataset = create_dataset(test_opt)
            model.opt.is_train = 0
            model.opt.no_loss = 1
            test(model, test_dataset, Visualizer(test_opt), test_opt, test_bg_info, test_steps=resume_iter,idx = idx) # 59
            opt.resume_iter = str(int(opt.resume_iter) + 1)
    torch.cuda.empty_cache()
