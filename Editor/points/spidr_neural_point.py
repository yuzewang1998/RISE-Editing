...
class SpidrNeuralPoint(BaseNeuralPoint):
    ...
    def set_input(self,points_xyz,**kwargs):
        ...
        self.roughness = kwargs['points_roughness']

    def load_from_ply(self, name):
        self.ply_path = os.path.join(self.file_dir, name + '_spidrneuralpoint.ply')
        ...
        self.roughness = np.array(plydata.elements[0].data["roughness"].astype(np.float32))[..., None]
    ...
    def save_as_ply(self,name):
        ...
        vertex = np.concatenate([self.xyz,self.color,self.conf,self.roughness,self.dir,self.embeding],axis=-1)
        ...
        self.save_ply_path = os.path.join(self.file_dir, name + '_spidrneuralpoint.ply')
    ...
    def select_from_meshlabpoint(self,meshlab_point):
        ...
        neural_ptr = create_neural_point(self.opt,'spidr')
        neural_ptr.set_input(points_xyz=self.xyz[id_list],points_embeding=self.embeding[id_list],points_conf=self.conf[id_list],points_dir=self.dir[id_list],points_color=self.color[id_list],points_roughness=self.roughness[id_list])
        return neural_ptr
    ...
    def __sub__(self, other:BaseNeuralPoint)->BaseNeuralPoint:
        ...
        neural_ptr.set_input(self.xyz[not_id_list], points_embeding=self.embeding[not_id_list],
                             ...
                             points_color=self.color[not_id_list],points_roughness=self.roughness[not_id_list])
        return neural_ptr

    def __add__(self, other):
        new_ptr = create_neural_point(self.opt, 'spidr')
        ...
        new_roughness = np.concatenate((self.roughness,other.roughness), axis=0)
        ...
        new_ptr.set_input(new_xyz, points_embeding=new_embeding,
                             ...
                             points_color=new_color,points_roughness=new_roughness)
        return new_ptr
