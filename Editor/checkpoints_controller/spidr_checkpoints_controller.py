...
import numpy as np
import os
class SpidrCheckpointsController(BaseCheckpointsController):
    ...
    def cvt_2_neuralPoint(self):
        ...
        self.points_roughness = self.network_paras["neural_points.points_roughness"].view(-1, 1).cpu().numpy()
        print('point cloud scale:', self.points_xyz.shape, type(self.points_xyz))
        neural_point = create_neural_point(self.opt,'spidr')
        neural_point.set_input(self.points_xyz,points_embeding=self.points_embeding,points_conf=self.points_conf,points_color=self.points_color,points_dir=self.points_dir,points_roughness = self.points_roughness)
        return neural_point
    def set_and_save(self,pointnerf_neuralpoint=None,edit_name = None,ani_flag = 0):
        ...
        if pointnerf_neuralpoint is not None:
            ...
            self.network_paras["neural_points.points_roughness"] = torch.unsqueeze( torch.Tensor(pointnerf_neuralpoint.roughness[..., np.newaxis]), dim=0)  # [1,ptr,1]
