...
class NeuralPoints(nn.Module):
    ...
    def grow_points(self, add_xyz, add_embedding, add_color, add_dirx,add_diry,add_dirz, add_conf, add_label,add_eulers=None, add_Rw2c=None):
        ...
        if self.points_conf is not None:
            ...
            self.points_conf.requires_grad = self.opt.conf_grad > 0
        if self.points_dirx is not None:
            self.points_dirx = nn.Parameter(torch.cat([self.points_dirx, add_dirx[None, ...]], dim=1))
            self.points_dirx.requires_grad = self.opt.dir_grad > 0
        if self.points_diry is not None:
            self.points_diry = nn.Parameter(torch.cat([self.points_diry, add_diry[None, ...]], dim=1))
            self.points_diry.requires_grad = self.opt.dir_grad > 0
        if self.points_dirz is not None:
            self.points_dirz = nn.Parameter(torch.cat([self.points_dirz, add_dirz[None, ...]], dim=1))
            self.points_dirz.requires_grad = self.opt.dir_grad > 0
    ...
    def get_point_indices(self, inputs, cam_rot_tensor, cam_pos_tensor, pixel_idx_tensor, near_plane, far_plane, h, w, intrinsic, vox_query=False):
        ...
        ray_label_tensor= inputs["raylabel"] if inputs["raylabel"]  is not None else None
        # (1,784,3)-784个采样点
        ...
        sample_ray_labels_tensor = None
        if ray_label_tensor is not None:
            sample_pidx_tensor, sample_loc_tensor, sample_loc_w_tensor,sample_ray_dirs_tensor,sample_ray_labels_tensor, ray_mask_tensor, vsize, ranges = self.querier.query_points(pixel_idx_tensor, point_xyz_pers_tensor, self.xyz[None,...], actual_numpoints_tensor, h, w, intrinsic, near_plane, far_plane, ray_dirs_tensor, cam_pos_tensor, cam_rot_tensor,self.points_label[None,...],ray_label_tensor)
        else:
            sample_pidx_tensor, sample_loc_tensor, sample_loc_w_tensor, sample_ray_dirs_tensor,sample_ray_labels_tensor, ray_mask_tensor, vsize, ranges = self.querier.query_points(pixel_idx_tensor, point_xyz_pers_tensor, self.xyz[None, ...],actual_numpoints_tensor, h, w, intrinsic, near_plane, far_plane, ray_dirs_tensor,cam_pos_tensor, cam_rot_tensor)
